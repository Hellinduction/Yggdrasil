...
public class StartSessionCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        final List<Player> playerPool = plugin.isCullingSession ? players.stream()
                .filter(player -> PlayerData.retrieveLives(player.getUniqueId()) == 0)
                .collect(Collectors.toList()) : players;

        if (plugin.isCullingSession && playerPool.isEmpty()) {
            sender.sendMessage(ChatColor.RED + "Unable to start cullying session with no dead players.");
            return true;
        }
        ...
        if (plugin.isCullingSession) {
            Bukkit.broadcastMessage(ChatColor.DARK_RED + "" + ChatColor.BOLD + "A culling session has begun amongst dead players.");
            ...
            players.forEach(player -> {
                ...
                player.sendMessage(ChatColor.GOLD + "Since your still alive, you do not get to participate in the culling session.");
            });
        }
